...
# Use debug for development to enable hot reloading and assertions
echo "[ios:prepare-rust] Building Rust core static library (arm64 simulator)..."
cargo build --manifest-path "$ROOT_DIR/crates/core/Cargo.toml" --target "$SIM_TARGET_ARM64"

echo "[ios:prepare-rust] Building Rust core static library (x86_64 simulator)..."
cargo build --manifest-path "$ROOT_DIR/crates/core/Cargo.toml" --target "$SIM_TARGET_X86_64"

mkdir -p "$IOS_LIB_DIR"
lipo -create \
  "$ROOT_DIR/target/$SIM_TARGET_ARM64/debug/libbefu_core.a" \
  "$ROOT_DIR/target/$SIM_TARGET_X86_64/debug/libbefu_core.a" \
  -output "$IOS_LIB_DIR/libbefu_core.a"

echo "[ios:prepare-rust] Synced $IOS_LIB_DIR/libbefu_core.a (DEBUG mode)"
